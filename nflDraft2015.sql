/* SQLite */
/* Top 99 2015 NFL Draft Picks as curated by Pamela Fox */
...
INSERT INTO picks (pick_number, name, college, picked_by, position)
	VALUES(6,"Leonard Williams","USC","New York Jets","DE");
INSERT INTO picks (pick_number, name, college, picked_by, position)
...
INSERT INTO picks (pick_number, name, college, picked_by, position)
	VALUES(37,"Devin Smith ","Ohio St.","New York Jets","WR");
INSERT INTO picks (pick_number, name, college, picked_by, position)
...
INSERT INTO picks (pick_number, name, college, picked_by, position)
	VALUES(82,"Lorenzo Mauldin","Louisville","New York Jets","OLB");
INSERT INTO picks (pick_number, name, college, picked_by, position)
...
--All data for all picks
SELECT * FROM picks;

--Average pick for colleges
SELECT college, ROUND(avg(pick_number)) AS avg_pick FROM picks GROUP BY college ORDER BY avg_pick;

--Number of picks for each college
SELECT COUNT(*) AS numPicks, college FROM picks GROUP BY college ORDER BY numPicks;

--Schools with 3+ picks
SELECT college, COUNT(*) AS numPicks FROM picks GROUP BY college HAVING numPicks >= 3;

--Team's first picks
SELECT picked_by, min(pick_number) AS best_pick FROM picks GROUP BY picked_by ORDER BY best_pick;

--Display picks by draft round
SELECT name, picked_by,
    CASE
...
--Defensive Tackles drafted by the Patriots in top 100
SELECT name, position FROM picks WHERE picked_by = "New England" AND position = "DT";
